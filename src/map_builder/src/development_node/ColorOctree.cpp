...
class Octree{
    ...
    void insertNode(float x, float y, float z, uint8_t r, uint8_t g, uint8_t b){
        ...
        if(current->Occ == -1){         // to make sure we don't label them as unknown 
            current->Occ =1; 
...
int main(){
    ...
    int count =1; 
    // while(x<8.00f){
    //     tree.insert(x, x, x, 10, 10, 10); 
    //     x+=0.05f; 
    //     count+=1; 
    // }

    ifstream ifile;
    ifile.open ("nodes.txt", ios::in);
    float x_map, y_map, z_map;  
    int r_map, g_map, b_map; 
     
        
    while(ifile >>x_map>>y_map>>z_map>>r_map>>g_map>>b_map){
        tree.insert(x_map, y_map, z_map, uint8_t(r_map), uint8_t(g_map), uint8_t(b_map));  
        count +=1; 
    }
    ifile.close(); 
    ...
    while(!leaf_nodes.empty()){
            ...
            std::cout<<current->X<<"\t"<<current->Y<<"\t"<<current->Z<<"\t"<<unsigned(current->Occ)<<
                    "\t"<<unsigned(current->R)<<"\t"<<unsigned(current->G)<<"\t"<<unsigned(current->B)<<endl;    
            leaf_nodes.pop(); 
    }	 
    return 0;
}