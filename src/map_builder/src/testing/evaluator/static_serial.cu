...
int cudamain(sensor_msgs::PointCloud2 point_cloud_std, nav_msgs::Odometry odom_message_std){ 
    ...
    uint64_t *mcode_arr; 
    uint32_t *rgbo_arr; 
    int size_mcode_arr = array_size * sizeof(uint64_t);
	int size_rgbo_arr = array_size * sizeof(uint32_t);
	mcode_arr = (uint64_t *)malloc( size_mcode_arr );
    rgbo_arr = (uint32_t *)malloc( size_rgbo_arr );
    // ---------------------------------------------------------------------------------------------------------------------
    double start3, end3; 
    start3 = clock();
    for(int i=0; i<array_size; i++){
        mcode_arr[i] = generate_morton_code(x_rounded[i], y_rounded[i], z_rounded[i]);
    }
    end3 = clock();
    ...
    for(int j=0; j<array_size; j++){
        rgbo_arr[j] = create_node_value(r[j], g[j], b[j], 0b00010001); // occupied
    }
    end4 = clock();
    ...
    for(int i=0; i<array_size; i++){
        ...
        if(morton_code != 0b0000000000000000100011011011011011011011011011011011011011011011){
            ...
            if (itr == octree.end()){
                ...
                value = rgbo_arr[i] & 0b11111111111111111111111100000000;
    ...
    std::cout<<time1<<"\t"<<time2<<"\t"<<time3<<"\t"<<time4<<"\t"<<time5<<"\t"<<timetotal<<std::endl;
