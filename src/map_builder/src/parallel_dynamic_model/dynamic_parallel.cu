...
__global__ void generate_node_arrays(float* x, float* y, float* z, 
	...
	if( (fabs(x[index]) < max_free_radius) and (fabs(y[index]) < max_free_radius) and (fabs(z[index]) < max_free_radius)){
...
int cudamain(sensor_msgs::PointCloud2 point_cloud_std, nav_msgs::Odometry odom_message_std, int size){ 
    ...
    generate_node_arrays<<<NUM_OF_BLOCKS, THREADS_PER_BLOCK>>>(d_x, d_y, d_z, 
	...
	if(offile.is_open()) { 
		while(!nodes.empty()){
            current = nodes.front(); 
            if(signed(current->Occ) > -16){
                //std::cout<<signed(current->Occ)<<std::endl; }
